...
import { api } from "../../api/client";            // Shared fetch wrapper (adds the base URL and Bearer token).
      ...
      <section className="panel-right">                  {/* Scrollable column of student cards. */}
        ...
        {users.map((u) => (                              // Render one card per user.
          <div className="user-card" key={u.id}>         {/* key = user id so React tracks rows efficiently. */}
            <img
              src={u.profile_photo_url || `${import.meta.env.BASE_URL}favicon.svg`} // Avatar, or the brand mark as a fallback.
