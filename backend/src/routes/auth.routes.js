...
router.post("/register", async (req, res) => {                 // Handle POST /auth/register.
  // CHANGE (photo release): phone_country_code and phone_number are pulled from the
  // body as well. Register.jsx now collects them, and they are PRIVATE — stored on
  // the users row but never selected by GET /users, which feeds the public panel.
  const { email, password, first_name, last_name, phone_country_code, phone_number } = req.body; // Pull the fields the form sends.
  ...
  // Length guards mirroring the column widths (VARCHAR(8) / VARCHAR(30)). Catching
  // this here returns a readable message instead of Postgres error 22001.
  if ((phone_country_code || "").length > 8) {                // Dialing code too long for the column...
    return res.status(400).json({ error: "phone_country_code too long" }); // ...say exactly which field is wrong.
  }
  if ((phone_number || "").length > 30) {                     // Phone number too long for the column...
    return res.status(400).json({ error: "phone_number too long" });       // ...same clear rejection.
  }
    ...
    const { rows } = await pool.query(                         // Insert the new user and return only safe columns.
      `INSERT INTO users (email, password_hash, first_name, last_name,
       ...
       RETURNING id, email, first_name, last_name,
                 phone_country_code, phone_number`,            // RETURNING avoids a second SELECT and never returns the hash.
      [email.toLowerCase(), hash, first_name, last_name,       // Lowercase email so "A@x.com" and "a@x.com" can't both register.
       phone_country_code || null, phone_number || null]       // Empty strings become NULL so "no phone" is stored consistently.
    );
