    ...
    const { rows } = await pool.query(                           // Run the UPDATE limited to the caller's own row.
      `UPDATE users SET ${setClause}
       ...
       RETURNING id, first_name, last_name, current_country, current_city,
                 ...
                 profile_photo_url,
                 phone_country_code, phone_number`,              // RETURNING gives the client the updated profile without a 2nd query. CHANGE (photo release): the two private contact columns are now returned as well, so Profile.jsx keeps showing what the user just typed once App.jsx merges the response into its user state. Safe to return here because this row belongs to the caller — GET / below still excludes them.
      values
    );
